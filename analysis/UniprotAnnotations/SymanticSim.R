...
#remove same ID-same ID instances
term_term <- term_term[which(term_term$Var1 != term_term$Var2),]
term_term <- term_term[which(term_term$value != 0),]

###too many terms! try slims

###############################
####ATTEMPT 2 with slim terms##
################################
...
write.csv(edge_attb, "~/Documents/GitHub/OysterSeedProject/analysis/UniprotAnnotations/GOsemsim_edges/edge_attb_semsim0.7.csv", row.names = FALSE, quote = FALSE)


##############################
####ATTEMPT 3 with cutoffs and half matrix
################################
...
write.csv(term_term, "~/Desktop/term_term.csv", row.names = FALSE, quote = FALSE)
