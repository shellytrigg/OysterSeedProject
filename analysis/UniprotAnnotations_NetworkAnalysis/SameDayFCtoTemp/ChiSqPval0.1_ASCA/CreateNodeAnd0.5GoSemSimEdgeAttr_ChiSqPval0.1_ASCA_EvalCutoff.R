...
all_sig0.1_pro_logFC_pval_abbrv <- all_sig0.1_pro_logFC_pval[,-c(1,20:44)]
#add column to describe node type for cytoscape
all_sig0.1_pro_logFC_pval_abbrv$type <- "protein"

#remove non-adj.pvals
all_sig0.1_pro_logFC_pval_abbrv <- all_sig0.1_pro_logFC_pval_abbrv[,-grep("_ChiSq", colnames(all_sig0.1_pro_logFC_pval_abbrv))]

#save node attribute file
write.table(all_sig0.1_pro_logFC_pval_abbrv,"~/Documents/GitHub/OysterSeedProject/analysis/UniprotAnnotations_NetworkAnalysis/SameDayFCtoTemp/ChiSqPval0.1_ASCA/all_sig0.1_pro_logFC_pval_abbrv_Evalcutoff_NodeAttb.txt", sep = "\t",quote = FALSE, row.names = FALSE)
...
write.csv(STACKED_sig0.1_pro_GOid_term, "~/Documents/GitHub/OysterSeedProject/analysis/UniprotAnnotations_NetworkAnalysis/SameDayFCtoTemp/ChiSqPval0.1_ASCA/intermediate_files/STACKED_sig0.1_pro_GOid_term.csv", quote = FALSE, row.names = FALSE)
...
#721;not bad because there were 729 initially
...
#[1] 130; all proteins with GO IDs mapped to at least 1 slim term
...
write.table(edge_attb, "~/Documents/GitHub/OysterSeedProject/analysis/UniprotAnnotations_NetworkAnalysis/SameDayFCtoTemp/ChiSqPval0.1_ASCA/edge_attb_semsim0.5_sig0.1ASCA_EValcutoff.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
for (i in 1:length(sig0.1_GOslims_terms_uniq)){
  ...
  for(j in days){
    ...
    mag_sum <- sum(abs(all_sig0.1_pro_logFC_pval[which(all_sig0.1_pro_logFC_pval$gene %in% protein_list),paste("D_",j,"_logFC",sep = "")]))
    mag_df[1,paste("D",j,"_magFC", sep = "")] <- mag_sum
...
go_term_magFC <- merge(go_term_magFC, norm2prot, by = "term")
